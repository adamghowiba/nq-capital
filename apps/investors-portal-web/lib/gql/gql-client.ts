...
export const MeSettingsDocument = `
    query MeSettings {
  meInvestor {
    ...InvestorAllFragment
  }
...
export const useMeSettingsQuery = <
      TData = MeSettingsQuery,
      TError = unknown
    >(
      variables?: MeSettingsQueryVariables,
      options?: Omit<UseQueryOptions<MeSettingsQuery, TError, TData>, 'queryKey'> & { queryKey?: UseQueryOptions<MeSettingsQuery, TError, TData>['queryKey'] }
    ) => {
    
    return useQuery<MeSettingsQuery, TError, TData>(
      {
    queryKey: variables === undefined ? ['MeSettings'] : ['MeSettings', variables],
    queryFn: gqlFetcher<MeSettingsQuery, MeSettingsQueryVariables>(MeSettingsDocument, variables),
    ...options
  }
    )};

useMeSettingsQuery.document = MeSettingsDocument;

useMeSettingsQuery.getKey = (variables?: MeSettingsQueryVariables) => variables === undefined ? ['MeSettings'] : ['MeSettings', variables];


useMeSettingsQuery.fetcher = (variables?: MeSettingsQueryVariables, options?: RequestInit['headers']) => gqlFetcher<MeSettingsQuery, MeSettingsQueryVariables>(MeSettingsDocument, variables, options);
